...
interface = Tk()
interface.title("Mile to Km Converter")
interface.minsize(width=400, height=200)
interface.config(padx=100, pady=50)


def miles_to_km():
    km_result_label.config(text=round(float(mile_entry.get()) * 1.6, 2))
...
is_equal_to_label = Label(text="equals to")
...
# def btn_clicked():
#     print("I got clicked!!")
#     # my_label.config(text="Btn got clicked")
#     my_label.config(text=in_put.get())
#
#
...
#
# new_btn = Button(text="Twice", command=btn_clicked)
# new_btn.grid(row=0, column=2)
#
# in_put = Entry(width=10)
# in_put.grid(row=2, column=3)
# in_put.get()
# window.mainloop()
